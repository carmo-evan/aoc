...
func getMultipliedPair(input []int, target int) int {
	m := make(map[int]int)
	for i, v := range input {
		candidate := target - v
...
func getMultipliedTriplet(input []int, target int) int {
	m := make(map[int]int)
	for i, a := range input {
		for j, b := range input {
			if j == i {
